...
def render_cascade(workspace: Workspace) -> str:
    ...
    lines = [
        ...
        f'2. Start a run trace under `{runtime}/runs/{{date-slug}}/` in the invocation runtime workspace.',
        f'3. Read `{runtime}/config.json`; if missing, invoke the config command atom and scaffold an empty phase config.',
        '4. Run lifecycle phases in order: ' + ', '.join(f'`{phase}`' for phase in workspace.phases) + '.',
        ...
        f'10. Command skills are public atoms; `{entry_invocation(workspace)}` may invoke them when orchestration requires it.',
