...
| `code-documentation` | `intents` | keep code-adjacent documentation current<br>document touched public or reusable code surfaces<br>explain dense implementation logic |
| `code-documentation` | `surfaces` | exported symbols and package entry points<br>component inputs, routes, commands, hooks, services, and providers<br>DTOs, schemas, payloads, generated docs, README usage, Storybook, and MDX<br>long or branch-heavy functions |
| `code-documentation` | `risks` | changed behavior lacks consumer-facing documentation<br>comments narrate syntax instead of preserving intent<br>stale examples or docs remain reachable when code has moved |
...
| `code-documentation` | `repo` | diff creates, edits, moves, splits, deletes, or materially reviews code<br>diff changes a public import, framework binding, configuration, command, or user workflow<br>diff touches ordering-sensitive, lifecycle-sensitive, cached, concurrent, retrying, paginated, or error-recovery logic |
