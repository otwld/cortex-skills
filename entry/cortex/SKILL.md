...
3. Read `.codex/config.json`; if missing, invoke the Cortex config command atom to scaffold the default phase config and record the bootstrap in the run trace.
...
7. Add phase-specific `always` atoms from `.codex/config.json` for the current phase.
...
- `.codex/config.json` was read or scaffolded through the config command atom.
