...
drift as soon as repository evidence invalidates the plan.

## Workflow

1. Convert the plan into a visible task checklist and keep status current.
2. Execute tasks in the planned order unless current evidence proves the order is
   invalid.
3. Keep behavior, tests, docs, migration notes, and generated outputs in the same
   task when they govern the same change.
4. Protect unrelated worktree changes. Read them when needed for context, but do
   not revert, reformat, or include them in the executed scope.
5. Run validation named by the plan at the point it names; use project-native
...
planned change requires a new product, API, ownership, or validation choice; a
task cannot be completed without editing out-of-scope paths; or validation fails
for a reason that changes the plan rather than the implementation detail.
...
- Plan drift, blockers, or deviations and the evidence behind them.
