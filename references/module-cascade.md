...
Use this reference from `$cortex` when a request touches multiple domains or
when module selection is not obvious from one explicit Cortex request.

## Cascade Algorithm

1. Collect direct signals from the request, files, diff, config, and intended operation.
2. Convert direct signals to root modules through the signal rules below.
3. Expand recursive `BEFORE` edges from `references/module-graph.md`.
...
| Signal | Evidence | Start Modules |
| --- | --- | --- |
| Explicit Cortex request | User includes `$cortex` and names a workflow or provides task evidence | `cortex` plus evidence-backed modules |
...
- Do not make `cortex` a hard parent of every module through `BEFORE`.
- Do not use `WITH` as automatic expansion.
- Honor explicit user limits on modules or tools.
