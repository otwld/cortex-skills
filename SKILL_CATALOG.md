...
Cortex Skills contains 3 public skills and 43 internal modules grouped by
reusable purpose. The public `cortex` router and explicit setup command skills
have `SKILL.md` and `agents/openai.yaml`; internal modules use `MODULE.md` and
are routed only after `$cortex` is invoked. Command skills under
`governance/setup/` are user-invoked only and excluded from Cortex module
routing.
...
| `agent-instructions-bootstrap` | Explicit-only command skill for bootstrapping or auditing workspace AI-agent instruction files. | `governance/setup/agent-instructions-bootstrap/` |
| `project-memory-setup` | Explicit-only command skill for setting up or verifying durable project-memory artifacts. | `governance/setup/project-memory-setup/` |
